function [ON_C50, OFF_C50, ON_Slope, OFF_Slope, da, db,Stats] = analysePysch(Names)
...
for i = indx
    ct = ct+1;
    B=load([path MouseID Folders{i} '/Data.mat'])
    PS = getPsychometric(B.data, 1, corrflag);
    ...
    %
    %     [ONMdl,param,stat]=sigm_fit(xax,On(ct,:),[],[min(On(ct,:)),max(On(ct,:)),0.5,4],1);
    %     ON_C50(ct) = stat.param(3);
    %     ON_Slope(ct) = stat.param(4);
    %     R2On(ct) = ( corr(stat.ypredupperCI,On(ct,:)')).^2;
    %
    %
    %     [OFFMdl,param,stat]=sigm_fit(xax,Off(ct,:),[],[min(Off(ct,:)),max(Off(ct,:)),0.5,4],1);
    %     OFF_C50(ct) = stat.param(3);
    %     OFF_Slope(ct) = stat.param(4);
    %     R2OFF(ct) = (corr(stat.ypredupperCI,On(ct,:)')).^2;
end;
...
% [ONMdl,param,stat]=sigm_fit(xax,median(On),[],[],0);
% ON_C50 = param(3);
% ON_Slope = param(4);
%
% [OFFMdl, param,stat]=sigm_fit(xax,mean(Off),[],[],0);
% OFF_C50 = param(3);
% OFF_Slope = param(4);
